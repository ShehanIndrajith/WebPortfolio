...
function navpanel({ scrollToSection, refs, active }) {
    const { aboutRef,experienceRef, educationRef, projectsRef, certificationsRef } = refs;
  return (
    <motion.div initial={{ y: -20, opacity: 0 }} whileInView={{ y: 0, opacity: 1,  delay:0.5 }} transition={{ duration: 0.5, ease: "easeInOut" }} viewport={{ once: true }} className='flex flex-row flex-wrap rounded-full p-2 mt-2 w-Full h-12 bg-white/5 backdrop-blur-sm justify-center items-center space-x-2'>
      ...
      <div onClick={() => scrollToSection(experienceRef, "Experience")} className={`rounded-full items-center py-1 px-3 hover:bg-white/10 hover:cursor-pointer ${
          active === "Experience" ? "hover:bg-white bg-white text-black" : ""
        }`}>
        <a>Experience</a>
      </div>
